...
// each Gaussian.
__device__ void
computeColorFromSH(int idx,//正在处理的高斯函数的索引
                   int deg,//球谐函数的阶数
                   int max_coeffs,//系数的最大数量
                   const glm::vec3 *means,//高斯函数均值位置的数组
                   glm::vec3 campos,//摄像机的位置
                   const float *shs,//球谐系数的数组
                   const bool *clamped,//指示在正向传播期间是否对颜色进行了截断的数组
                   const glm::vec3 *dL_dcolor,//损失函数相对于 RGB 颜色的偏导数
                   glm::vec3 *dL_dmeans,//损失函数相对于均值位置的偏导数
                   glm::vec3 *dL_dshs//损失函数相对于球谐系数的偏导数
                   ) {
    // Compute intermediate values, as it is done during forward
    glm::vec3 pos = means[idx];//高斯函数的均值位置
    glm::vec3 dir_orig = pos - campos;//从摄像机到高斯函数的原始方向向量
    glm::vec3 dir = dir_orig / glm::length(dir_orig);//经过归一化后的方向向量

    glm::vec3 *sh = ((glm::vec3 *) shs) + idx * max_coeffs;//指向球谐系数数组中当前高斯函数位置的指针

    // Use PyTorch rule for clamping: if clamping was applied, gradient becomes 0.
    //对 RGB 颜色的损失梯度。根据是否应用了截断，其中的每个分量可能会被置为 0
    glm::vec3 dL_dRGB = dL_dcolor[idx];
    dL_dRGB.x *= clamped[3 * idx + 0] ? 0 : 1;
    dL_dRGB.y *= clamped[3 * idx + 1] ? 0 : 1;
    dL_dRGB.z *= clamped[3 * idx + 2] ? 0 : 1;

    //RGB 颜色相对于空间位置 x、y 和 z 的梯度。在这里，它们被初始化为零
    ...
    //将球谐梯度写入的目标位置，即 dL_dsh 数组的索引，该数组存储每个高斯函数的球谐梯度
...
__global__ void computeCov2DCUDA(int P,//高斯数量
                                 const float3 *means,//每个高斯的均值
                                 const int *radii,//每个高斯的半径
                                 const float *cov3Ds,//每个高斯的 3D 协方差矩阵
                                 const float h_x, float h_y,//像素宽度和高度
                                 const float tan_fovx, float tan_fovy,//水平和垂直方向上的焦距
                                 const float *view_matrix,//视图矩阵
                                 const float *dL_dconics,//损失函数相对于 conic 参数的梯度
